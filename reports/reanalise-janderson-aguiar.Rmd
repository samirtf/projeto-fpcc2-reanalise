...
comparacao2_a = merged %>% 
    filter(algoritmo %in% c("alpha0.5---w-1-1-1", "alpha0.5---w-1-1-0"))
...
theta2_a <- function(d, i) {
    agrupado = d %>% 
        ...
        summarise(media = mean(rmse))
    b = agrupado %>% filter(algoritmo == "alpha0.5---w-1-1-1") %>% pull(media)
    l = agrupado %>% filter(algoritmo == "alpha0.5---w-1-1-0") %>% pull(media)
    l - b
}
...
df2 = df2 %>% add_row(diferenças = "alpha0.5---w-1-1-0 -- alpha0.5---w-1-1-1", estatística = ci2_a$statistic, inferior = ci2_a$conf.low, superior = ci2_a$conf.high)
